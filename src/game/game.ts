...
import { defaultParty, isDown, allDown, countItem, takeItem } from './party.ts';
...
export class Game {
  ...
  castExplore(casterIndex: number, spellId: string): void {
    ...
    switch (sp.explore) {
      ...
      case 'town_portal': this.say('Nothing happens yet.'); break;
      default:
        if (sp.heal) { const t = this.party.members[this.selected]; this.say(`${c.name} casts ${sp.name} on ${t.name}.`); this.push(new MessageScreen('')); this.pop(); }
    }
  }
