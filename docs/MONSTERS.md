# The Monsters of Caldera

*What the company fights down the road, area by area, and what each thing it fights tells it about
the world. DESIGN.md says what happens and STORY.md how it should feel; this is the creature half of
every area's brief (EXPANSION.md §8.2), to be carried into `docs/areas/<area>.md` as each area is
built. It is a draft, to take in or leave: where it and DESIGN.md disagree, the design wins. Figures
are measured on this branch at `5092f9c`.*

---

## 1. What the monsters are for

Three jobs. Where they pull apart, the earlier one wins.

1. **They are the gate.** Nothing on the road is locked (EXPANSION.md §2.2): what turns a company
   back is the monsters. An area's monsters let a company at its band's floor through and turn one
   two levels under it back (EXPANSION.md §5.2), and they do it by what they are and who they stand
   with, not by their numbers alone (DESIGN.md §6).
2. **They carry the secret.** The road starts among beasts and people and ends among machines, and
   nobody tells the company why. Each act shows a little more: the first thing that does not bleed,
   the first to carry the chisel's mark, a monk who walks like the keepers of the sleepers' bay.
   DESIGN.md §4 says that in the Underdeep the monsters stop being animals; that is the foot of a
   slope that starts on the Shelf.
3. **They are the land.** Each area's monsters come out of its ground, its weather and its hours:
   crows in the stubble, wreckers in the fog, pike under the ice, trolls that stand on the tors all
   day as stone.

The rules that follow from them:

- **Every monster is something.** Each is one of five kinds (§2), and its kind says what works on
  it. A company learns the rules on the Shelf and spends them all the way to the Core.
- **Meet it gentle first.** A family's first group on the road is its smallest and weakest, near
  the way in, so a company learns what the family does from a fight it can survive or run from.
  This is EXPANSION.md §5.2's warning, not a wall, for every family as well as every border.
- **Composition before numbers.** A hard fight is hard because of who stands with whom: the healer
  behind the soldiers, the leader the rest run without, the thicket in front of the oak. At most 12
  to a fight in at most three groups, and a bigger threat is a bigger monster (DESIGN.md §6).
- **Back changed, or not at all.** A family that returns brings new gear, anatomy or behaviour, as
  the wolf came back as the dire wolf and the rift hound; never the same drawing with bigger
  numbers. It is the anti-pillar against dead engine reuse, for monsters.
- **At most two or three new families an area** (EXPANSION.md §11), and variants on the frames that
  exist for the rest. §11 is the ledger.
- **The look says the role.** An archer carries a bow, a caster glows, a brute is big, a machine is
  smooth. The look is also the line the log says when a group is first seen, and every monster
  below has one, two lines of the log at most: it is how the danger is made legible (EXPANSION.md
  §7).
- **Every boss is a guardian.** It ends something or yields something, and never comes back
  (EXPANSION.md §5.1): the Hand of Ash yields the chisel, the Warden of the Cut the first Meridian
  journal, and each region's deepest boss can carry the next (DESIGN.md §10.3). It asks, too: won
  about half the time at the band's floor, nearly always two levels over (EXPANSION.md §5.2).
- **Found, never told.** No monster explains itself, and no fight's text names the machine. What a
  thing does and what it carries are the clue.
- **Plain names.** English words a listener could spell, as STORY.md is written for the ear, in
  British spelling. *Warden* is taken: Harrow's Wardens, the Wardstones and the Rift's bosses (the
  Rift Warden, the Warden of the Cut) keep it, and nothing else gets it. No name says ship, hull or
  crew.

---

## 2. The five kinds

**Beasts.** The land's animals and plants, grown large in the Might and Magic way: rats, wolves,
spiders, the old wood, eels, bears, drakes. Sleep takes them, and they run when most of their group
is down. They carry no gold. Each has its own resistances rather than its kind's: the fire beetle
shrugs off fire because it lives in it. *Underneath:* they came in the ship, as the people did.
First on the Shelf; the last are the rats of the Underdeep, which nobody brought.

**People.** Brigands, smugglers, wreckers, the Compact's crews, the dwarves if the company crosses
them, the giants of the Stair, and the Ashen Hand. Sleep takes them; they carry gold and gear; most
break when their leader falls, and the Hand never does. *Underneath:* the crew's descendants, like
the company. First on the Shelf; the last is Vask.

**The dead.** Skeletons, barrow guards, drowned men, wights, the ash-cast dead of Old Cinder. Holy
light and Holy Strike bite; sleep and the druid's swarm do nothing, and they never run.
*Underneath:* nothing. They are the one kind the machine does not explain, and they stay the
fantasy's own. The Lanterns say the dead walk where the light is thin, and it is true: they gather
where a Stone is cut, and lie down when it is restored.

**The Rift.** Riftlings, rift hounds and crawlers, the elders, the wardens. Fast and hot, and their
touch paralyses. Cold bites the hot ones and fire the cold (§2.1); sleep takes the small ones and
never a warden; they stop coming when their Stone is restored. *Underneath (offered):* a Stone's
field, torn loose at the cut and set round whatever lies near it: ember and crystal at the Grove,
brine at the Tide Stone, slag at the Anvil. Its heart is a shard of the Stone, and a warden's heart
is the biggest, the humming lump STORY.md finds by the cellar's tear. First in the cellar; the last
are the lights over the bog round the dead ring on Cairnmoor.

**The machine.** The knockers of the Deep Mines, the tallymen, the keepers of the sleepers' bay, the
monks of the Whitespine, the stokers of Fire Mountain, and the things that live below. Lightning
bites. Sleep, the swarm and the Hearth's light do nothing: Smite and Wrath of the Hearth pass
through a machine, and the log says so the first time (*The light goes into it like a hand into a
glove.*), because the gods are the Custodian's voices and these are its hands. They never run, and
carry no gold, only parts. *Underneath:* the vessel's hands, each still doing the job it was given
four hundred years ago: inspecting, counting, tending, stoking, guarding. None of them is wicked;
the company is in the way. First at the bottom of the Deep Mines; the last at the Core.

### 2.1 The Rift, Stone by Stone

Rifts open near a dead Stone and stop when it is restored (DESIGN.md §2), so each Stone's Rifts
wear its material, and the Rift generator (EXPANSION.md §7) can dress its templates from this
table: a Rift in the Kilns is slag, and a Rift in the Delta is brine.

| The Stone | Its Rifts | Material and light | What bites | Its things |
|---|---|---|---|---|
| The cellar's tear | the Shelf, under Ashcombe | ember and crystal, orange | cold | Riftling, Rift Crawler, the Rift Warden (built) |
| The Grove Stone, cut | Thornmark | ember and crystal; the Warden of the Cut burns blue | cold; fire on the Warden | Riftling Elder, Rift Hound, the Warden of the Cut (built) |
| The Tide Stone, stolen | the Delta, and the Tide Ship's hold | brine glass, sea green | lightning | Brineling, Tide Elder, the Warden of the Tide |
| The Sunder, a Stone that failed long ago | Sunderwood | black glass and dead wood, white | fire | Sunderling, Sunder Hound, the Warden of the Sunder |
| The Anvil Stone, cut by the dwarves | the Kilns | slag and iron, red | cold | Slagling, Slag Elder, the Warden of the Anvil |
| The stone ring, dead for centuries | Cairnmoor | no body left, only light | nothing in particular | Bog Light |
| The Peak Stone, whole | none | the Whitespine has no Rifts: its danger is the mountain and the monks | | |
| The Ember Stone, never finished | none | it never made a field to tear, so on the far side the machine is near the surface instead | | |

### 2.2 The kinds down the road

| | Beasts | People | The dead | The Rift | The machine |
|---|---|---|---|---|---|
| Act I | most of the groups | brigands, smugglers, wreckers, the Hand | barrows and drowned caves | the cellar and the Grove | none |
| Act II | many | the Compact's crews, the Hand's overseers and gleaners | the drowned temples | at its biggest: the Sunder | none, and a wall with nothing near it |
| Act III | fewer | the Hand's overseers with the cargo; the dwarves, if crossed | the oldest, in the cairns | its last: the Anvil, the ring's lights | the first: knockers, tallymen, keepers |
| Act IV | the mountain's and the far side's | the giants, the Hand's masons | the ash-cast of Old Cinder | none | robed at the monastery, bare at the vents |
| Act V | rats | the Hand's last, and Vask | none | none | nearly everything |

Two lines of it can be checked, and should be, beside the novelty check (EXPANSION.md §5.4): no
machine on the road before the bottom of the Deep Mines, and no Rift after Cairnmoor. They keep the
secret's pace. A company that goes straight down finds a mystery, not a spoiler (DESIGN.md §14.4):
the first machines stand in band-16 country, where a company that arrives early does not live long
enough to wonder what they are.

---

## 3. What a monster can do

### 3.1 Today

A monster's whole turn is `monsterAct` in `src/game/combat.ts`. It picks one member, from the front
row unless it is `ranged` or the front row is down, rolls to hit against that member's armour, and
deals its dice. A hit may add a condition at a chance (`inflict`: poison, disease, sleep or
paralysis); ten monsters use it, and none for sleep. `ranged` lets it pick the back row, `missile`
marks the bows the weather spoils, and `mindless` shrugs off Slumber. It carries gold and drops.

That is all. No monster casts, heals, calls for help, runs, regenerates, resists an element or
stands behind another; monsters stand in no rows, so a weapon in the party's front row reaches any
of them. Nothing gives a curse or stone, though both conditions exist: Restore lifts a curse, and a
temple a stoning.

### 3.2 What the numbers show

**Holy Strike lands on the wrong things.** The paladin's +3 is keyed to `mindless` (`traitDamage`),
and its text says the mindless dead. Nine monsters are mindless: the five dead, and the Cellar
Slime, the Shore Crab and both of the Rift's wardens, which are not dead. A machine family built on
`mindless` would take it too. `kind` (§3.3) fixes it.

**One spell answers every fight past the floor.** The damage spells that scale do so with the
caster's level (their dice times half the level, rounded up), and the widest reach every foe. At
level 8 Meteor Swarm averages 46 to every foe (8d10, and 2 for Cassian's Spellfire), and every
regular monster in Thornmark but the ogre has 45 hit points or fewer. At level 10 it averages 57,
and the premade Cassian has about 73 spell points, five castings (averaged over 200 rolls). That is
the difficulty plain EXPANSION.md §2.2 measures: past the floor, every fight is won, and won the
same way.

Meteor Swarm gains about 5.5 a level; the line's hit points (§4.1) gain 3. Steepening every
monster's hit points to outrun it would leave the company's weapons behind, since they grow with
gear and a point of Might every other level, not with level. So the road past level 10 needs
monsters that one spell does not answer: kinds and elements that make the company choose (§2),
ranks that shield a back row, abilities that make the order of the kills matter, and more hit
points on the few (the elites, the brutes, the bosses) than a single casting takes. Whether a
spell's dice should stop growing somewhere is the systems lane's to weigh. The combat harness puts
numbers on it (§4.4): for a company to fight its standard encounters between rests, a monster of
its level needs about twice the line's hit points from level 8. Past 16, on today's rules, no split
of hit points and damage keeps the days safe: fights the weapons can finish once the spells are
spent kill, and fights that do not kill stall. Stop the spells growing at 10 and let something else
grow past it, the gear, a bonus every two levels or a blow more a turn at 11 and another at 29, and
the days are safe again; the blows keep them safest. Where spells stop, and what grows in their
place, are for the spells past 10 and the prestiges to settle (#20, #19).

### 3.3 What the road asks

Each ask is a field on `MonsterDef` or `EncounterDef` and a few lines of `combat.ts` or `world.ts`,
made once in the systems lane and first spent by the area named, which waits for it (EXPANSION.md
§3, principle 2); no area invents one on the side. Each lands with the gate bot's answer to it
(EXPANSION.md §5.2), or the check will call a monster harder than it is.

| Ask | What it is | First needed | For |
|---|---|---|---|
| `level` | the monster's level, held to its map's band, give or take two | now | EXPANSION.md §5.2 |
| `kind` | beast, person, dead, Rift or machine, setting §2's defaults; a def may add its own (the slime and the crab keep their immunity to sleep) | now | Holy Strike on the dead only; the machines, by the Kilns at the latest |
| `look` | the line said the first time a group of it is seen, and the art's brief in one sentence | the Downs | the danger made legible (EXPANSION.md §7) |
| `when` | a group that walks only by night, in fog or snow, or in a season, read from the clock and the weather, so nothing new is saved | the Downs | the wreckers, the black dog |
| `until`, `after` | a group that stops coming back once a Stone is restored, or comes only after a step or a choice. Neither opens or shuts a way, so neither is a story lock (EXPANSION.md §2.3) | Thornmark | its Rifts closing with the Grove Stone; the old wood sleeping; the Anvil Guard; the Underdeep noticing |
| Ranks | a group's front and back: melee reaches the back once the front is down, and bows, slings and spells reach anyone | Saltreach, or Thornmark's retune if it wants it first | DESIGN.md §6's caster behind a wall of shields, which cannot be built today |
| Elements | fire, cold, lightning, nature and holy on the damage spells; `resist`, `immune` and `weak` on a monster | Saltreach, or the Deepthorn for fire on the wood | the first area past level 10, where one spell stops being every answer |
| Morale | people break when their leader falls and beasts when three in four are down, and leave the fight; the Hand, the dead, the Rift and the machine never do | Saltreach | the Barge Master and his crews |
| Casting | a monster's turn spent on a spell from the tables: Mend or Bless on its group, Fire Bolt or Slumber on a row | Saltreach | the drowned chanters, the menders, the Hand's preachers |
| Drain | its hits heal it, or take spell points instead of hit points | Saltreach; spell points on Cairnmoor | the leeches; the bog lights |
| Regeneration | heals a set amount each round, except a round it took fire | Cairnmoor | the trolls |
| Curse and stone | `inflict` takes cursed and stoned, and stone gets a cure short of a temple | Cairnmoor for curse, the Wold for stone | the wights; the basilisk |
| Calls | a turn spent bringing a named group into the fight, within 12 and three groups | Rimewater | the tallymen; Vask |
| Sweep | one attack at every member of a row, with an element if it has one | the Whitespine; with fire, Ashfall | the giants; the drakes' breath |
| A light seen past the fog | a monster carrying a light is seen a square further than the weather allows (wanted, not needed) | the Downs | the lampmen |
| The Core's clock | the last fight counts rounds to the light going out | the Core | STORY.md, Act Five |

---

## 4. The numbers

### 4.1 The line

Give each monster the level of where it stands (the middle of its map's band, the gentlest a little
under), take out its role (§4.2), and the 28 regulars on the road today sit on one line. At level
*L* a soldier has about **3L + 4** hit points, armour **11 + L/2**, to-hit **1.5 + L/2**, and hits
for **3 + L/2** on average:

| Level | Hit points | Armour | To-hit | Damage |
|---|---|---|---|---|
| 2 | 10 | 12 | 2.5 | 4 |
| 8 | 28 | 15 | 5.5 | 7 |
| 16 | 52 | 19 | 9.5 | 11 |
| 24 | 76 | 23 | 13.5 | 15 |
| 32 | 100 | 27 | 17.5 | 19 |

A character's to-hit gains half a point a level (`attackBonus`), so armour on the line keeps the
chance of hitting a soldier the same at every level: Bram 75%, Wren 80% and Cassian 60%, at level 2
and at level 10 alike. A company's armour grows with its gear, not its level, so the line's to-hit
is what makes gear matter: in his starting gear, a soldier on the line hits Bram 38% of the time at
level 2 and 57% at level 10. The gear the curve gives a company at each level (EXPANSION.md §5.2)
is what holds that.

The line is where an area starts, not where it ends: the gate check tunes it, and what it tunes is
a cost. What a company at the floor has over one two levels under it is mostly hit points and spell
points: the premade six have about 155 and 90 between them at level 5, and 105 and 55 at level 3
(averaged over 200 rolls), with spell tier 3 landing between, at level 4. An area holds its gate
when the fights of its road, from one rest to the next, cost more than the smaller pool and less
than the larger. A group's cost is the damage it deals in the rounds it lasts, so its damage and
its staying power are two halves of one number, which the combat harness measures (§4.4).

### 4.2 Roles

Measured on today's monsters against the line:

| Role | Hit points | Armour | To-hit | Damage | Speed | Today |
|---|---|---|---|---|---|---|
| Fodder | ×0.6 | −1 | −1 | ×0.6 | 12 and up | the Giant Rat; six or more to a group |
| Skirmisher | ×1 | = | = | ×0.95 | 14–17 | Wolf, Dire Wolf, Rift Hound |
| Soldier | ×1 | = | = | ×1 | 10–11 | Bandit, Smuggler, Brigand, Zealot |
| Archer | ×0.85 | = | +0.5 to +1 | ×0.85 | 11–12 | the archers and bowmen: `ranged` and `missile` |
| Caster | ×0.95 | = | +1 to +1.5 | ×1.05 | 11–12 | Acolyte, Adept: `ranged` |
| Controller | ×0.8 to 1.25 | = | = | ×0.65 to 1 | any | the spiders, the Ghoul, the Wraith: a condition at 0.1 to 0.4 a hit |
| Armoured | ×1.25 | +1.5 to +2.5 | = | ×1.05 | 8–9 | Shore Crab, Bone Knight |
| Elite | ×1.45 | +0.5 | +1 | ×1.25 | 15 | Riftling Elder |
| Brute | ×1.9 | −1.5 | +1 | ×1.45 | 8 | the Ogre, alone or with two or three |
| Boss | ×2.5 to 4.2 | +1 to +2 | +1.5 to +2.5 | ×1.5 to 1.8 | 12–13 | the five, from the Rift Warden to the Warden of the Cut |

The road adds three roles on a soldier's numbers: the **leader**, whose fall breaks its group; the
**healer**, which mends its own; and the **caller**, which brings another group.

### 4.3 Pay, and size

**Pay.** Experience and gold are the curve's (EXPANSION.md §5.2): a clear pays for its band whatever
the area's size, so what a monster pays comes from its area first and its role second. Inside an
area pay follows risk. Thornmark's, for scale: the dire wolf and the thorn spider 120–130, soldiers
and archers 140–190, the adept and the rift hound 240–250, the bone knight and the wraith 280–330,
the elder 440, the ogre 520, and the bosses 2,000 and 3,000. Beasts carry no gold, and machines
carry parts, which is one place the Ember Stone's can come from (DESIGN.md §9).

**Size.** Today it runs from 0.35, the rat, to 1.25, the ogre. A lone monster stands 34 + 92 × size
pixels (`combatHeight`) in a view 268 high: the ogre 149, a size of 2 about 218, four-fifths of the
view, and 2.5 fills it. A pair stands as tall as one. That is room for the bigger monsters of
DESIGN.md §6: the heartwood, the giants, the drakes, the crawlers below.

### 4.4 The test monster, and the fights between rests

The yardstick: **a company can fight six or seven standard encounters at its own level between
rests**, to level 10, and a fight more every four levels after as its power grows: eight at 16,
twelve at 32, never fifteen. Its spell points count as much as its hit points. It must rest once it
has lost someone, once anyone is still under a quarter of their hit points after it has mended what
it can, or once it is under a quarter of its spell points. No fight goes on for ever: one still
running after fifteen rounds is broken off, and the company rests. A standard encounter is eight
fodder; four skirmishers, soldiers, archers or controllers; three casters or armoured; two elites or
brutes; or the boss alone. A mix is one encounter when its shares add to one: two soldiers and four
fodder, say.

`tools/testmonster.ts` stats a generic monster for every role at every level, and `tools/harness.ts`
measures it. The company is the premade six at the level, in the gear the tables give it by then
(the kits, the Shelf's mid-tier from 4, Thornmark's armoury from 8). It fights the encounter again
and again from a fresh start, mending between fights with spells as a player would, until it loses
one or must rest. A bot plays it that mends whoever is in danger and strikes, and casts a damage
spell only when the hit points it saves are worth the spell points. Each is counted against what
the company has left of that pool before it must rest, and a healer's spell points are priced at
what they would mend. So the bot spends on what hurts while it is fresh, and holds back as its
spell points run down. The calibration:

- Every role keeps its shape (§4.2) and hits as today's monsters hit, on the line; its hit points
  are whatever makes the day's fights.
- Where those fights would run too long on average, hit points hold them there and damage rises
  instead. Too long is four rounds at level 1 and a round more every eight levels, as both sides
  grow: five at 9, eight at 32.
- Where the days then end badly more than one day in ten, in a death, a lost fight or one broken
  off, the calibration steps the way that helps. Where members die or fights are lost, the fights
  run longer and hit softer, up to half as long again (six rounds at level 1, twelve at 32). Where
  fights are broken off, they run shorter and hit harder, never past two and a half times the
  line's damage. It takes the first step that ends no more than one day in ten badly, or else the
  step that ends fewest.
- Where a role on the line already leaves the company short of its fights, it comes down whole, hit
  points and damage together.
- No monster has fewer hit points than the one a level under it, at the levels made or between.
- The boss, one monster acting once a round against six, has both raised together until a company
  two levels under it wins half the time (EXPANSION.md §5.2).
- It is made at every level to 10 and every fourth level to 32, 300 seeds a point, with the levels
  between interpolated. Past 10 the company runs on today's rules extended, with no new spells,
  prestiges or gear, so that stretch is provisional.

Hit points / average damage a hit, by role and level:

| Level | Fodder | Skirmisher | Soldier | Archer | Caster | Controller | Armoured | Elite | Brute | Boss |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | 3 / 1.5 | 6 / 3 | 7 / 3.5 | 6 / 3 | 8 / 3.5 | 7 / 2.5 | 13 / 3.5 | 12 / 4.5 | 26 / 5 | none |
| 2 | 6 / 3 | 13 / 4 | 15 / 4 | 14 / 3.5 | 21 / 4 | 13 / 4.5 | 18 / 8 | 26 / 5.5 | 35 / 8 | none |
| 3 | 7 / 4 | 17 / 4.5 | 17 / 5.5 | 16 / 4 | 24 / 4.5 | 14 / 5.5 | 20 / 7 | 32 / 7 | 41 / 9 | 111 / 18.5 |
| 4 | 13 / 4.5 | 24 / 6.5 | 23 / 8 | 24 / 4.5 | 31 / 7 | 20 / 7 | 29 / 11 | 50 / 10 | 67 / 11 | 138 / 21 |
| 5 | 17 / 5 | 27 / 8 | 26 / 9 | 26 / 6.5 | 33 / 8 | 25 / 8 | 34 / 10 | 52 / 11.5 | 80 / 11.5 | 174 / 24 |
| 6 | 22 / 3.5 | 31 / 8 | 29 / 9 | 29 / 7 | 38 / 9 | 29 / 7 | 35 / 10 | 53 / 11 | 82 / 11 | 222 / 29.5 |
| 7 | 27 / 4 | 39 / 6.5 | 35 / 9 | 39 / 5.5 | 45 / 8 | 34 / 9 | 41 / 11.5 | 58 / 12 | 83 / 11 | 265 / 33 |
| 8 | 47 / 4.5 | 57 / 9 | 59 / 9 | 56 / 9 | 70 / 9 | 57 / 10 | 70 / 11 | 88 / 13 | 119 / 13 | 311 / 37.5 |
| 10 | 55 / 10 | 79 / 8 | 79 / 10 | 80 / 8 | 94 / 9 | 77 / 7 | 84 / 13 | 103 / 14 | 143 / 13 | 412 / 47 |
| 12 | 69 / 12 | 91 / 9 | 84 / 14 | 83 / 13 | 117 / 9 | 82 / 13 | 98 / 15 | 121 / 14 | 147 / 14.5 | 531 / 58 |
| 16 | 100 / 11 | 108 / 16.5 | 107 / 21 | 105 / 18 | 125 / 17.5 | 104 / 20 | 116 / 27 | 136 / 19.5 | 198 / 16 | 819 / 84 |
| 20 | 137 / 11 | 132 / 19.5 | 144 / 24 | 141 / 21 | 156 / 25 | 128 / 23 | 165 / 14 | 188 / 22 | 235 / 19 | 1008 / 99 |
| 24 | 231 / 9 | 185 / 17 | 185 / 25 | 205 / 21 | 189 / 29 | 182 / 20 | 252 / 15.5 | 216 / 28 | 277 / 22 | 1162 / 111 |
| 28 | 288 / 10 | 243 / 18 | 279 / 25.5 | 282 / 25 | 283 / 30 | 274 / 22 | 310 / 18 | 221 / 33 | 301 / 40.5 | 1300 / 122 |
| 32 | 334 / 11.5 | 341 / 18 | 363 / 19 | 345 / 25 | 342 / 32 | 333 / 22 | 338 / 41 | 344 / 34 | 365 / 37.5 | 1474 / 136 |

Armour and to-hit are the line's, rounded, plus the role's offsets in §4.2: armour 12 and to-hit 2
at level 1, 16 and 7 at 10, 27 and 18 at 32. Speeds are the roles': fodder, archers, casters and
controllers 12, skirmishers and elites 15, soldiers 11, the armoured and brutes 8, the boss 13.
Archers and casters reach the back row, archers with bows; controllers and elites paralyse, 0.15 a
hit. Each monster pays its share of an encounter, and an encounter pays a twentieth of what the
company needs for its next level (`FIGHTS_PER_LEVEL`, the curve's to set), about three rests' worth:
75 at level 1, 885 at 10, 2,865 at 32. There is no boss under level 3, since a boss sits at its
band's top.

What it shows:

- **The day ends on a wound at levels 1 and 2, and on spell points from 4.** Low down, the healers'
  few points go first while the sorcerer still holds most of its own, and the day ends with someone
  left under a quarter. From 4 to 12, four days in five or more end with the company under a quarter
  of its spell points, for every role but the elites and brutes at 4. Level 3 is where they cross.
- **The line is already a day's fighting at level 1.** Eight fodder, or four skirmishers, archers or
  controllers, on the line leave a company of 1 short of six or seven fights, so they come down
  whole, to between 0.8 and 1 of the line.
- **Held to short fights, monsters hit harder than today's.** From level 3 to 12 most roles have 1.1
  to 3.1 times the line's hit points and hit up to 1.8 times as hard, the armoured up to 2.1. A
  company that husbands its spells through the day kills slowly, and a fight that must end in four
  or five rounds has to take its share of the day in them.
- **Some fights cannot be finished.** A fight still going at fifteen rounds is broken off, and it
  happens when the spells are spent and the monsters have more hit points than weapons get through.
  At 9–12, where fodder need a lot of hit points to live through a spell that takes the whole group,
  eight of them would stall one day in two; so there they carry fewer and hit twice as hard.
- **The heavy hitters are slower.** Held to four rounds, brutes, elites and the armoured hit hard
  enough to take a member from standing to −10 in one blow, and a death ended up to half their days
  at 4–12 (two brutes at 4, nearly half). Given half as long again, a death or a lost fight ends
  4–9% of the armoured's days, 4–15% of the elites' and 2–21% of the brutes', in fights of 4.4 to
  7.5 rounds. Two brutes at 4 are the worst of it, a fifth of the days even so.
- **Hit points jump where tier 5 lands.** At 8, where Meteor Swarm, Tempest and Wrath reach every
  foe, every role needs nearly half as many hit points again as at 7, or more (the soldier 35 to
  59). Tier 3 at 4 shows most on fodder, since Fire Bolt and the swarm take a whole group of them: 7
  hit points at 3, 13 at 4.
- **Today's maps are close, and a little hard low down.** Their own groups, dealt in a new order
  each day, give a company at its band's floor four to eight fights before a rest: the Shelf 4.6 at
  level 1 and the Ashcombe cellar 5.9, Greywater's caves and the Drowned Shrine 4.1 at 2 and 3,
  Thornmark 5.7 at 5, the Grove Roots 6.0 at 6 and the Cut Stone 7.7 at 8. On the Shelf the cliff
  smugglers are a day's work on their own for a company of 1 (1.6 fights); at the Cut Stone, Meteor
  Swarm ends most fights in a round or two. The round cap barely touches them: no more than one day
  in fifty ends with a fight broken off.
- **One fight from a fresh start costs about a tenth.** To level 12 a standard encounter costs a
  fresh company 6–15% of its hit points and spell points, 7–13% at most points, and past it, where
  rests come further apart, 3–9%. So 15% a fight, the first yardstick, was near; but a rest at a
  quarter, and a company that saves its spells, make the day shorter than a hundred over fifteen.
- **A lone boss has to kill with a blow.** At level 8 it hits for 37.5 against a company whose
  sorcerer has about 24. Real bosses want an escort, as the Hand of Ash and the Warden of the Cut
  have, or the sweep of §3.3.
- **Past 12, on today's rules, no monster makes a good day.** With rests further apart, monsters
  there need only 1.7 to 4 times the line's hit points (fodder up to 5.6), where six or seven fights
  a rest asked up to seven. But the company's spells grow with its level and nothing else does, so
  once they are spent its weapons cannot finish a fight, and blows hard enough to end fights sooner
  kill. From 16 a sixth to two-fifths of the days end badly for most roles, half for elites by 28,
  nine in ten for brutes at 28, and nearly all for the armoured at 20–28, whom weapons barely
  scratch.
- **Weaker spells, and something else that grows.** Past 10 the balance can come from the monsters,
  the spells or the company, and the harness tries each. `--spell-cap 10` stops damage spells
  growing at 10. `--gear-grows` enchants Thornmark's gear past 10, so that a weapon's blow grows as
  the line's hit points do and armour gains a point every two levels, as the line's to-hit does.
  `--level-bonus` gives every member a point of weapon damage and of armour every two levels
  instead. `--level-traits` gives the fighters (in the premade six the knight, paladin, ranger and
  thief) a blow more a turn at 11 and another at 29, where the promotions the prestiges replaced
  were to come (DESIGN.md §5), and grows the thief's sneak attack by 2 every two levels.
  Days that end in a death, a lost fight or one broken off, from 12 to 32:

  | Spells | What else grows past 10 | Most roles | The armoured |
  |---|---|---|---|
  | keep growing | nothing (today) | a sixth to two-fifths | nearly all |
  | keep growing | gear | a fifth to five in six from 24 | nearly all |
  | keep growing | a bonus every two levels | up to six in seven by 28 | nearly all from 24 |
  | keep growing | a blow at 11 and 29 | a fifth at most to 16, up to two-thirds from 24 | a tenth at most to 16, up to nine in ten from 24 |
  | stop at 16 | nothing | 1–33% | 58–83% |
  | stop at 16 | gear | 9–42% | 27–39% |
  | stop at 10 | nothing | a sixth at most (casters at 32, 22%) | 12–45% |
  | stop at 10 | gear | a sixth at most (fodder up to a third) | 4–12% |
  | stop at 10 | a bonus every two levels | a fifth at most (fodder up to a third) | 5–15% |
  | stop at 10 | a blow at 11 and 29 | a tenth at most, 8% past 12 | 12% at 12, then 4% at most |
  | stop at 10 | a blow at 11 and 29, and a bonus | a sixth at most (brutes 23% at 20) | 3–10% |

  While spells keep growing, none of them keeps the days safe. Past 16 or 20 the bad days are, as
  today, fights broken off once the spells are spent and deaths where monsters hit harder to end
  fights sooner. Gear and a bonus make it worse, since armour that keeps up means monsters must hit
  harder still to take their share of the day. With spells stopped at 10, each of them works.

  A bonus every two levels does what growing gear does. The two calibrate within a few hundredths
  of each other: every role at 1.6 to 3 times the line's hit points, hitting on it or up to 1.6
  times as hard, in fights of two to nine rounds. Only fodder, which must hit twice as hard to
  matter against good armour, still end up to a third of their days badly. Which to give is a
  matter of feel, loot or levels, not of balance.

  A blow more at 11 and at 29 does best. Monsters carry 1.6 to 3.7 times the line's hit points
  and hit on it from 24 (up to 1.6 times as hard at 12), in fights of three to seven rounds: the
  blows add no armour, so monsters need not hit harder to take their share of the day. The first
  blow has to come early. Given at 21 and 32 instead, the blows leave 11 to 20 with nothing new, and
  at 20 a quarter of the controllers' days and over a third of the armoured's end badly. Between
  11 and 29 the company gains little, and from 16 the calibration would give five roles in nine
  fewer hit points than the level under them at one level or more; held at the level under's,
  brutes at 24 and 28 leave the company a fight or two short of its day (8.6 of 10, 9.2 of 11). The
  bonus on top is worse, not better: monsters hit up to 1.6 times the line, fights run to twelve
  rounds, and brutes end up to a quarter of their days badly. The prestiges that replace the
  promotions are drafted at 11, 19 and 27, with perks and spell ranks rather than spell tiers
  (DESIGN.md §5, §7; #19, #20); the harness runs them in place of `--level-traits` once they are
  settled.

`node tools/harness.ts` reports every role's standard encounter at every calibrated level: the
fights before a rest, what ended the day, the rounds a fight took, what one fight from a fresh start
costs, and the boss's odds. `--map thornmark --level 5` puts a map's own groups against a company
of that level, each on its own and then all of them dealt in a new order. `--stats` prints the stat
lines with their dice, and `--calibrate --write` re-derives the tables when the rules change.
`--spell-cap 10`, `--gear-grows`, `--level-bonus` and `--level-traits` run any of it as if damage
spells stopped growing at 10, or the company gained gear, a bonus or blows as it levelled past it;
they combine, and play has none of them.

---

## 5. Act I: Ashes on the Road (levels 1–10)

*Why are the Stones failing?* Every kind but the machine is met here, each at its gentlest, and the
act ends on a Stone cut with tools from below.

### 5.1 The Shelf (band 1–5; built)

| Kind | On the Shelf, in the cellar and at Greywater |
|---|---|
| Beasts | Giant Rat, Cellar Slime, Wolf, Wild Boar, Marsh Spider, Shore Crab |
| People | Bandit, Bandit Archer, Smuggler, Smuggler Bowman, the Smuggler Captain; and the Hand's first ranks, the Ashen Cultist, the Ashen Acolyte and the Ashen Deacon |
| The dead | Skeleton, Drowned Man, Ghoul |
| The Rift | Riftling, Rift Crawler, the Rift Warden |

It stays as built. What this asks of it:

- `kind` on every def: the slime and the crab are beasts, the Rift Warden is the Rift, and Holy
  Strike stops landing on them.
- The Rift Warden leaves its heart behind: the humming shard STORY.md finds by the tear, the first
  thing on the road with the Hearth's hum in it, heard again under the Hearth.
- The rats are the first thing most companies kill. The road brings them back twice, aboard the
  Tide Ship and at the bottom of the world (§9.2).

### 5.2 Harrow Downs (band 2–5; the pilot)

*The chalk hills west of the Shelf, the fields round Gullwick, and the coast past Harrow Light to
the Ledge. The Shelf's weather: mild, wet, and foggy off the sea in the autumn.*

The Downs are the night the light went out, seen from the shore. The fishermen steer by the Hearth;
with it failing the coast is dark, and the dark has people in it who were waiting for it (STORY.md,
Act One). Up on the hills the Queen's barrow stands open, and her guard is up. The Downs are the
pilot's zone map (EXPANSION.md §9, Phase 1), so their monsters test the pipeline and not the
resolver: one new family, variants on the frames that exist, and two small fields.

**New:** birds, the frame the road reuses for owls, herons, gulls, ravens, eagles and vultures.
**Back:** the Wolf on the hills, bandits and their archers on the coast road, the Shore Crab under
Harrow Light.

| Monster | Family | Role, level | Where | Look; what it does |
|---|---|---|---|---|
| Carrion Crow | birds, new | fodder, 2 | the stubble fields; the gibbet on the coast road | *Crows, too many to count, and all of them watching.* Flies, so it reaches the back row (`ranged` without `missile`); six to eight to a group |
| Wrecker | bandit | soldier, 3 | the coast from Gullwick to Harrow Light, by night and in fog | *Oilskins, a boathook, and a boat they were expecting.* Carries a chit stamped with the Harrow customs seal, as the Greywater crates are |
| Lampman | bandit | archer, 3 | one to every wreckers' group | *A lantern held high on a pole, and someone under it.* Slings stones over the wreckers; their leader, once there is morale |
| Barrow Guard | skeleton | armoured, 4 | the Queen's barrow | *The Queen's guard, in her colours, still standing to.* Halberds; holds its ground and never roams |
| Black Dog | wolf | skirmisher, 4 | the hills round the barrow, by night | *A black dog the size of a calf, with eyes like coals.* The dead's own hound: its bite holds (paralysis, 0.1) |
| Barrow Captain | skeleton | boss, 5 | at the Queen's empty bier | *Her captain, at his post beside an empty bier.* |

- **Fog on the coast road.** A lampman and four wreckers on a foggy night, when thick fog leaves
  two squares of sight (SLICE.md). The first fight where the weather is a warning and not a
  nuisance: the lamp is what the company sees first.
- **Crows over wolves** in the stubble: the back row learns it can be reached.
- **The barrow**: guards two by two down the passage, then the captain at the bier. It is the
  Downs' hardest place, band 4–5, and its boss asks there.

**Asks:** `look` and `when`. Flyers are `ranged` without `missile`, which exists.

### 5.3 Thornmark (band 5–10; built, retuned in the pilot)

*The old forest over the pass: Thornhold, the old tower, the barrow, the river, and the Grove.*

Thornmark is where the Stone is cut, and every kind the Shelf showed is here at full strength:
packs of dire wolves, spiders that poison, brigands with archers, the Hand in numbers, the barrow's
knights and wraiths, and the Rift the cut let through. The roster stays. What the pilot changes is
the gate: today a company wins nine fights in ten here at level 3 (EXPANSION.md §2.2), and the gate
wants one in four at most. The monsters' levers, in order:

- **The gentlest first.** The first groups past the pass are wolves and brigands, and the crossing
  line says how the land feels (EXPANSION.md §5.2).
- **Groups that ask.** The adepts behind the zealots, once there are ranks; the brigands' archers
  with the ogre; thorn spiders poisoning the front row before the dire wolves come in.
- **The cost, between the pools** (§4.1), set with `tools/gate.ts`.

When the Grove Stone is restored, Thornmark's Rifts close (DESIGN.md §9): its rift hounds and
riftling elders stop coming back, and so do the barrow's dead (`until`).

### 5.4 The Deepthorn (band 8–10)

*The oldest of the forest, south of Thornmark down to Thorn Head, and the oldest elf-hold, which
keeps the treaty. Thornmark's winter: cold, and snow that lies for weeks.*

The Deepthorn is age: trees older than Harrow in a wood older than the elves' memory, and a treaty
sealed with the chisel's mark, the first time the machine's script is seen on something that is not
a tool. Its monsters are the wood. The old wood woke when the Grove Stone was cut, and it does not
know friends from cutters; when the Stone is restored it sleeps again (`until`), and the elves can
walk their own paths.

**New:** the old wood. **Back:** the Dire Wolf in bigger packs, the Thorn Spider, the Ogre alone in
the deep; the Great Owl from the birds.

| Monster | Family | Role, level | Where | Look; what it does |
|---|---|---|---|---|
| Bramble | old wood, new | controller, 8 | across the deep paths | *A thicket that closes behind you.* Never roams, and wakes at one square; its thorns hold (paralysis, 0.3); slow |
| Great Owl | birds | skirmisher, 8 | the deep, by night | *Wings as wide as a cart, and not a sound.* Flies, and hunts the back row |
| Rootwalker | old wood, new | armoured, 9 | the old groves | *A stump walking on its roots, its bark like plate.* Slow and hard to hit; fire bites, once there are elements |
| Heartwood | old wood, new | brute, 10 | alone, or over a thicket of brambles | *An oak that has decided to move.* Size 1.8 |
| The Eldest | old wood, new | boss, 10 | Thorn Head | *The oldest tree in Caldera, and it is awake.* Asleep with the rest of the wood once the Grove Stone is restored |

- **Brambles and an owl** on a path at night: members held in the thorns while the owl takes the
  back row.
- **The oak behind the thicket**: a heartwood over three brambles, the brute behind a wall that
  holds.

**Asks:** `until` and `when`; elements, if the wood is to burn before Saltreach.

---

## 6. Act II: The Salt Road (levels 10–16)

*Who profits?* The act follows the cargo down the Long Water, out to the smugglers' ship and back
to the Sunder, and ends on Vask's name and a wall no one built. Its monsters are the first past
today's level cap, and the first that one spell must not answer.

### 6.1 Saltreach (band 10–12)

*The Long Water's country: the Upper Water's willows and Reedholm's fields, the Delta's fen and the
Drowned Temples, the Saltings' pans and Saltmouth, the free port. Salt flats and tidal ground.*

Saltreach has lost its Stone. The Tide Stone went downriver on a barge at midsummer, so its Rifts
are open and the fen glitters with brine glass; the temples are dark, and their drowned priests
count instead of singing; the Compact's barges carry shards and people down the Long Water. Most of
the road's new abilities are first spent here, which adds to what Saltreach already waits on in
Phase 2 (EXPANSION.md §7, §9).

**New:** long bodies (the eels first; later the worms and the pike) and toads. **Back:** the
Drowned Man in the temples; the Rift, in brine (§2.1); the spider frame as the salt crab; the Grey
Heron from the birds.

| Monster | Family | Role, level | Where | Look; what it does |
|---|---|---|---|---|
| Fen Eel | long bodies, new | skirmisher, 10 | in the river and the fen's water | *A back as thick as a man's leg, turning in the reeds.* Never roams; strikes at one square |
| Leech | long bodies, new | controller, 10 | the fen's pools | *Black, and as long as your arm.* Its hits heal it; six or more |
| Grey Heron | birds | skirmisher, 10 | the Upper Water | *Taller than Bram, and it has been watching.* Flies, and spears the back row |
| Fen Toad | toads, new | controller, 10 | the Delta | *A toad the size of a sheep, its skin weeping.* Poisons (0.3) |
| Bull Toad | toads, new | brute, 12 | the Delta, alone | *It could swallow Ottilie whole.* Size 1.4; carries disease |
| Bargeman | bandit | soldier, 10 | the barges on the Long Water | *A barge pole, a knife, and no questions.* Breaks when the master falls |
| Barge Master | bandit | leader, 11 | one to a barge | *A ledger in one hand and a cudgel in the other.* |
| Salt Crab | spider | armoured, 11 | the salt pans | *White with salt, and it glitters when it moves.* |
| Brineling | riftling | skirmisher, 10 | the Delta's Rifts | *Green glass, walking, with a light in it.* Lightning bites; cold does not |
| Drowned Chanter | skeleton | caster, 11 | the Drowned Temples | *A priest of the drowned god, still chanting.* Sings a row to sleep (Slumber); the chant is a count, and it ends on eleven |
| The Choirmaster | skeleton | boss, 12 | the Drowned Temples' choir | *The choir's master, beating time on a bell.* Blesses its group each round it can; when it falls, the count stops |

- **A barge at dusk**: a master and five bargemen, and two herons over the reeds. The master is the
  answer: the first fight on the road won by who falls first.
- **The fen's pools**: leeches with an eel in the water beside the path, one group mending itself
  as it hits and one out of reach until it strikes.
- **The choir**: four chanters behind two drowned men, the front row asleep while the chanters
  count.

The count stops with the Choirmaster, and the song comes back only with the Stone (STORY.md, Act
Two).

**Asks:** elements, ranks, morale, casting, drain.

### 6.2 Wrackholm (band 12–14)

*The smugglers' isle, reached by a smugglers' boat from Saltmouth: a heather moor with a rocky east
end, Smugglers' Cove, and the Tide Ship at anchor under the cliffs.*

Wrackholm is the cargo. The Tide Ship's hold carries shards and people bound below; the Compact's
crews work it, the Hand's overseers keep the chains, and the orders come up from below. In the
forward hold the stolen Tide Stone glows through its sacking, and its Rift is there with it.

**New:** devilfish, the old sailors' word for the octopus. **Back:** the rat as the bilge rat;
smugglers in better gear; the Hand as its overseers; the Tide Stone's Rift, with an elder and a
warden in the hold.

| Monster | Family | Role, level | Where | Look; what it does |
|---|---|---|---|---|
| Wrack Gull | birds | fodder, 12 | the cliffs | *A thousand gulls, and all of them angry.* Flies; eight to a group |
| Bilge Rat | rat | fodder, 12 | the Tide Ship's lower decks | *The rats aboard have been eating well.* Disease (0.15); eight to a group |
| Wrack Smuggler | bandit | soldier, 12 | the cove, and the ship's deck | *A Compact knife in a Harrow coat.* |
| Wrack Bowman | bandit | archer, 12 | with the smugglers | *A bow, and the high ground.* |
| Ashen Overseer | cultist | controller, 13 | the hold, among the chained rows | *Grey to the wrist, and a chain in each hand.* The chain holds (paralysis, 0.25) |
| Devilfish | devilfish, new | controller, 13 | the cove's pools; under the ship | *Arms, coming up over the side.* Reaches the back row; holds (paralysis, 0.2) |
| Tide Elder | riftling | elite, 13 | the forward hold | *A shard of the sea, stood up.* Paralyses (0.15) |
| Great Devilfish | devilfish, new | boss, 14 | Smugglers' Cove, the sea cave | *What the smugglers feed.* Size 1.9 |
| Warden of the Tide | riftling | boss, 14 | the forward hold, over the Stone | *The Stone's own light, standing guard over it.* The hold's tear closes when it falls |

- **The hold**: two overseers behind four smugglers, among the chained rows. The chains hold the
  front row while the bowmen shoot the back, and Hale is chained in the last row, watching.
- **Over the side**: two devilfish on the ship's waist at night, with a bowman on the rail.

**Asks:** none of its own: Wrackholm spends Saltreach's.

### 6.3 Sunderwood (band 14–16)

*The Eaves, pines under the rim on the Sunder's west side, and Lanternwood east of the gorge, with
Lantern Watch across the rope bridge. Along the Sunder, glass trees, crystal and dead wood, and a
gorge you cannot see the bottom of in the rain.*

Sunderwood is what a Stone's failure looks like at full size. The Sunder is the oldest and biggest
Rift in the world, and the beasts along it are changed by it: glass grows through their fur. At its
floor stands the wall, and nothing else: no group within four squares of it, because the silence
at the bottom is the monsters' part in the act's turn. In Lanternwood, the moths come to the
Lanterns' lamps.

**New:** moths, drawn to light, which come back in their thousands on Hearth Isle; and bears.
**Back:** the Rift, in black glass; the spider frame as the glass spider; the wolf frame as the
sunder hound; the Hand, gleaning shards.

| Monster | Family | Role, level | Where | Look; what it does |
|---|---|---|---|---|
| Pine Bear | bears, new | brute, 14 | the Eaves | *A bear, and then the rest of the bear.* |
| Lantern Moth | moths, new | fodder, 14 | Lanternwood, by night | *Wings like a pair of hands, and dust that glitters.* Flies; its dust puts to sleep (0.2) |
| Sunderling | riftling | skirmisher, 14 | the Sunder | *Black glass, with a white light inside.* Fire bites, for the dead wood in it |
| Glass Spider | spider | controller, 15 | the Sunder's edge | *Threads of glass, and something walking them.* Paralyses (0.25) |
| Sunder Hound | wolf | skirmisher, 15 | the Sunder | *A rift hound, gone to glass.* Paralyses (0.15) |
| Ashen Gleaner | cultist | soldier, 15 | the Sunder's ledges | *A sack of glowing shards, and a knife for the next.* The Hand quarries the Rift: every shard is a step |
| Deathshead | moths, new | elite, 16 | Lanternwood, by night | *A skull on its back, and a scream in its wings.* Flies; its dust puts to sleep (0.35) |
| Glass Bear | bears, new | armoured, 16 | the Sunder's floor | *Glass through its fur like frost.* Fire does not touch it |
| Warden of the Sunder | riftling | boss, 16 | the gorge, above the wall | *The Sunder's own knot, and it has held for centuries.* When it falls, the Sunder stays open |

The Warden of the Sunder is the first on the road whose death closes nothing: its Stone is long
gone, and some damage stays done.

- **Moths to the lamp**: lantern moths with a deathshead at night, half the company asleep in a
  round and the back row their first choice.
- **The gleaners' ledge**: gleaners with sunder hounds on a ledge a square wide.

**Asks:** `when`. Wanted: moths that come more often to a company carrying Light.

---

## 7. Act III: The Deep Script (levels 16–22)

*What are the Stones?* The machine is met, the dwarves' holy words turn out to be signs on doors,
and under the ice lie the sleepers. The act's monsters bring the machine up to the surface, one job
at a time.

### 7.1 The Kilns (band 16–18)

*The Iron Fells (Anvilhall and the Deep Mines), the Kilns' hill country (quarries, spoil, the Anvil
Stone, the lava tubes and their ash), and Kilnmouth (Kilnhaven, the ore port).*

The Kilns are where the machine is first met. Under the deepest mine the tunnel ends in a clean,
humming corridor, and the things that live in it come up into the mines, knocking on the rock as
they come: the dwarves call them knockers, and leave them a crust. They are small, many-legged and
smooth, with one lamp for an eye, and every one carries the chisel's mark. The dwarves cut their
own Anvil Stone to sell the pieces, so its Rift is here, in slag and iron, and the lava tubes run
with fire things.

**New:** knockers, the first machines, and salamanders. **Back:** the Rift, in slag; the long
bodies as the rock worm; the spider frame as the fire beetle; the Hand's overseers, walking the
cargo down the Deep Mines.

| Monster | Family | Role, level | Where | Look; what it does |
|---|---|---|---|---|
| Knocker | knockers, new | fodder, 16 | the Deep Mines' lowest levels | *Something small and grey, knocking on the rock as it comes.* A machine (§2); four to eight |
| Salamander | salamanders, new | skirmisher, 16 | the lava tubes | *A lizard with the fire showing through its skin.* Fire does not touch it; cold bites |
| Fire Beetle | spider | armoured, 16 | the spoil heaps and the forges | *A beetle with a coal in its back.* Fire does not touch it |
| Slagling | riftling | skirmisher, 16 | the Anvil Stone's Rift | *Slag, walking, with a red iron heart.* Cold bites |
| Mender | knockers, new | healer, 17 | with the knockers | *It stops to mend the others, and they let it.* Mends its group |
| Rock Worm | long bodies | brute, 17 | the old workings | *This tunnel was not dug by dwarves.* Size 1.6 |
| Anvil Guard | bandit, a dwarf on the figure frame | armoured, 17 | the Anvil Stone, if the company takes the Stone rather than buys it | *Short, broad, and in the thane's iron.* There only after the choice (`after`) |
| Slag Elder | riftling | elite, 18 | the Anvil Stone's Rift | *Iron runs off it like sweat.* |
| The Foreman | knockers, new | boss, 18 | the Deep Mines, before the door marked CREW ONLY | *It checks you the way a clerk checks a list, and finds nobody on it.* The door stays shut when it falls |
| Great Salamander | salamanders, new | boss, 18 | the lava tubes' deepest chamber | *The fire in the rock, with a head.* |
| Warden of the Anvil | riftling | boss, 18 | the Anvil Stone | *The Stone's heat, standing up out of the cut.* The tear closes when it falls |

- **The clean corridor**: six knockers and a mender. Kill the mender, or fight the rest twice; and
  the cleric's Wrath goes into them like a hand into a glove, the first time the road says so.
- **The spoil heaps**: fire beetles and salamanders, where the sorcerer's fire does nothing and the
  answer is Wren's Hailstorm.

**Asks:** `kind` for the machines, if it has not come before; `after`, for the Anvil Guard.

### 7.2 Cairnmoor (band 18–20)

*High Moor (heather, peat bog, tors and the stone ring) and the Cairnfield (the cairns). The company
crosses it in snow.*

Cairnmoor is the oldest ground on the road. The ring on the moor is older than the Wardstones, the
first emitters, dead for centuries, and inside it at night the Custodian's oldest voice still asks
the crew to report; the lights that drift over the bog are the last of the ring's field, the Rift
at its thinnest. The cairns hold the oldest dead, and the tors are trolls by night.

**New:** lights. **Back:** the ogre frame as the tor troll; the wraith's as the cairn wight; the
skeleton's as the bog body and the Cairn King; the wolf's as the moor hound; ravens from the birds.

| Monster | Family | Role, level | Where | Look; what it does |
|---|---|---|---|---|
| Raven | birds | fodder, 18 | the cairns | *They were here first.* Flies; eight to a group |
| Bog Light | lights, new | controller, 18 | over the bog by night, circling the ring | *A light over the bog, where nobody is.* Flies; its touch takes spell points, not hit points |
| Bog Body | skeleton | soldier, 18 | out of the peat | *Leather over bone, and a rope still round its neck.* Disease (0.2) |
| Moor Hound | wolf | skirmisher, 19 | the moor, by night | *The Downs' black dog, grown old and huge.* Paralyses (0.15) |
| Cairn Wight | wraith | controller, 19 | the Cairns | *A shroud with grave-gold at its throat.* Curses (0.2) |
| Tor Troll | ogre | brute, 19 | the tors, by night; by day they are stone, and have faces | *A tor that stood up.* Size 1.6; mends unless burned |
| The Cairn King | skeleton | boss, 20 | the oldest cairn | *Crowned, and older than the crown.* Curses |

- **Lights round the ring** on a snowy night, with a moor hound: the casters' spell points draining
  away while the hound runs at the front.
- **Trolls on the tor**: two trolls and a flock of ravens, and fire, or they get up again.

The ring itself holds no monster. Inside it there is only the voice.

**Asks:** regeneration, curse, drain of spell points, and `when`.

### 7.3 Rimewater (band 20–22)

*Longmere (the long lake, its meadows and Rime Lodge) and Coldmere (the frozen lake, with the sealed
bay under it); pinewoods, and the glacier's snow and ice. Glacier Foot, off Longmere, is the reach
(§10.2).*

Rimewater is the cargo coming home, and what follows it. The people who come up through the ice at
Rime Lodge were taken below, and the machine that counts the cargo comes up after them. Under
Coldmere the bay's keepers tend the sleepers, and try to put the company to bed beside them. These
are the first machines shaped like people, and the monks of the Whitespine wear the same frame
under their robes, so a company that fought the keepers will know the walk when it sees it again.

**New:** keepers, tall, thin, grey and many-fingered; and cats. **Back:** the knockers, and the
tallyman among them; the long bodies as the ice pike; the bears as the ice bear.

| Monster | Family | Role, level | Where | Look; what it does |
|---|---|---|---|---|
| Ice Pike | long bodies | skirmisher, 20 | under the lakes' ice | *The ice under Ottilie's feet has an eye in it.* Strikes a company on the ice, at one square |
| Snow Lynx | cats, new | skirmisher, 20 | the pinewoods | *Grey, tufted, and it has already jumped.* Leaps at the back row |
| Tallyman | knockers | caller, 20 | up through the ice-hole at Rime Lodge, by night | *It stops and clicks, once for each of you.* Calls a group of knockers |
| Ice Bear | bears | brute, 21 | the glacier's edge | *White, and bigger than the last one.* |
| Bay Keeper | keepers, new | controller, 21 | the sleepers' bay under Coldmere | *Tall and grey, with too many fingers, and gentle.* Its touch puts to sleep (0.3); mends its group |
| The Matron | keepers, new | boss, 22 | the bay's last row | *It has tended them for four hundred years.* |

- **The ice-hole**: a tallyman and six knockers up through the hole on the lodge's fourth night,
  with the lodge's fire at the company's back.
- **The bay**: three keepers among the beds, where anyone who falls asleep is a sleeper to them.

**Asks:** calls; a group placed on ice.

---

## 8. Act IV: Beyond the Sky (levels 22–28)

*What is Caldera?* The machine in a monk's robe, the Hand's road over the water, and a window onto
the outside of everything. There are no Rifts in this act: its Stones are whole, or never were.

### 8.1 The Whitespine (band 22–24)

*Monks' Vale (the monastery), the High Spine (the snows and the Peak Stone), Sheer Point (the Hand's
causeway), and the Giants' Stair, over the range and down into Ashfall.*

The Whitespine's Stone is whole, so its danger is the mountain, its giants and the monastery. The
monks died long ago, and the Custodian's hands keep their hours in their robes: they walk like the
keepers of the bay, speak the dwarves' old script to each other, do not bleed, and the Hearth's
light passes through them. Their bells ring the eleven. At Sheer Point the Hand builds its road of
shards out over the water.

**New:** giants. **Back:** the keepers, robed, as the monks; the ogre frame as the snow troll;
eagles from the birds; the Hand as its masons.

| Monster | Family | Role, level | Where | Look; what it does |
|---|---|---|---|---|
| Spine Eagle | birds | skirmisher, 22 | the High Spine | *It takes the light out of the sky as it comes down.* Flies |
| Brother | keepers | soldier, 22 | the monastery and Monks' Vale | *A monk, walking as if someone had described walking to it.* A machine under the robe |
| Bell-ringer | keepers | controller, 23 | the monastery's towers | *Eleven strokes, a gap, and eleven more.* Its bell holds (paralysis, 0.2) |
| Snow Troll | ogre | brute, 23 | the High Spine's snow | *A drift that stood up.* Mends unless burned |
| Stair Giant | giants, new | brute, 23 | the Giants' Stair | *A man as tall as a house, holding out his hand.* Size 2; sweeps the front row; asks a toll before it fights |
| Ashen Mason | cultist | soldier, 23 | Sheer Point, on the causeway | *A hammer from below, and a shard to set.* |
| The Abbot | keepers | boss, 24 | the monastery's chapter house | *The abbot keeps the hours, and it is time.* Its robe falls open as it falls |
| The Stair-king | giants, new | boss, 24 | the top of the Stair | *He has taken the toll here since before Harrow.* |

- **The chapter house**: brothers in front of two bell-ringers, the bells holding the front row
  while the brothers close, and the cleric's light doing nothing.
- **The Stair in snow**: a giant and a snow troll, one sweeping the front row and one getting up
  again.

The giants' toll is a choice before the fight, put the way a business puts its menu, so it needs
nothing new of the resolver; a company that pays walks up the Stair.

**Asks:** sweep.

### 8.2 Ashfall (band 24–26)

*Cindercoast (Cinderport, black sand, hanging vines, the hot springs), Fire Mountain (the vents, Old
Cinder, and Meridian Camp below them, with the window), and the Ember Waste, round the unfinished
Ember Stone.*

The Ember Stone was never finished, so the far side of the sea was never sealed, and the machine is
near the surface: the mountain's vents are the Underdeep's exhaust, and the stokers that tend them
walk out on the ash. Old Cinder is a town the mountain buried, its people cast in the ash. When the
company finishes the Stone, the Underdeep notices (DESIGN.md §9): every door below opens at once,
and what comes up hunts Ashfall from then on (`after`).

**New:** drakes, and the heavy machines. **Back:** the old wood as the strangler vine; the
salamanders; the spider frame as the cinder beetle; the dead, cast in ash.

| Monster | Family | Role, level | Where | Look; what it does |
|---|---|---|---|---|
| Strangler Vine | old wood | controller, 24 | the green shore's trees | *The vines hang lower than they did.* Never roams; holds (paralysis, 0.3) |
| Cinder Beetle | spider | armoured, 24 | the black sand | *Its shell is the colour of the beach.* |
| Ember Salamander | salamanders | skirmisher, 24 | the vents | *Grown in the mountain's own fire.* |
| Ash Husk | skeleton | soldier, 25 | Old Cinder | *A man of ash, still holding his cup.* |
| Stoker | heavy machines, new | brute, 25 | the vents | *It shovels nothing into nothing, and turns as you come.* Fire does not touch it |
| Cinder Drake | drakes, new | brute, 25 | Fire Mountain's slopes | *The mountain has children.* Flies; its breath burns a row |
| Sentry | heavy machines, new | elite, 26 | all of Ashfall, once the Ember Stone is lit | *It is looking for whoever touched the Stones.* |
| The Old Drake | drakes, new | boss, 26 | Old Cinder's crater | *The mountain's eldest, asleep on what is left of the town.* |
| The Sentinel | heavy machines, new | boss, 26 | the Ember Stone, the moment it lights | *The first thing up through the doors.* |

- **The vents**: two stokers with ember salamanders, where fire is useless and nothing but
  lightning troubles the machines.
- **After the Stone**: sentries on the road back to Cinderport, which was safe the day before.

**Asks:** sweep with an element; `after`.

### 8.3 The Wold (band 26–28)

*The Glasswold's steppe round the Glass, the Wold Riders' camp, dunes and mesas. The Glass itself is
the reach (§10.1).*

The Wold is the warning. The Riders remember the day the sky opened and the land burned to glass;
the grass has glass in it, the beasts that hunt it are the biggest on the road, and now and then
something walks out of the Glass that fell with the pillar of fire and has not stopped trying to
get up. The Riders keep the only way in, and they are right to.

**New:** glass walkers, the machines that fell with the lander, fused with the glass it made; the
Buried Tower is full of them. **Back:** the cats as the Wold's lions; the spider frame as the glass
scorpion; the salamanders as the basilisk; vultures from the birds.

| Monster | Family | Role, level | Where | Look; what it does |
|---|---|---|---|---|
| Vulture | birds | fodder, 26 | the steppe | *They land when something is about to die.* Flies |
| Wold Lion | cats | skirmisher, 26 | the steppe, in prides of three or four | *Tawny as the grass, and seen only when it moves.* |
| Glass Scorpion | spider | controller, 26 | the dunes | *Its sting is glass, and so is its shell.* Poisons (0.35) |
| Basilisk | salamanders | controller, 27 | the mesas | *Don't meet its eyes.* Turns a member to glass: stone (0.15) |
| Glass Walker | glass walkers, new | elite, 27 | the Wold's edge, by the Glass | *Something walking out of the Glass, and it has walked a long way.* A machine, and older than the rest |
| The Grey Lion | cats | boss, 28 | the Wold's heart | *Old, scarred, and king of all of this.* |

- **The mesa**: a basilisk behind a pride of lions, the lions keeping the front row busy while the
  basilisk glasses whoever looks up.

**Asks:** stone, and its cure.

---

## 9. Act V: The Hearth (levels 28–32)

*What do we do?* The Hand's last stand, the machine at work and nothing else, and at the heart of
it all, people.

### 9.1 Hearth Isle (band 28–30)

*A rocky rim round a green bowl, the temple built by people who thought the Hearth a god, and the
column of light itself.*

The causeway of shards lands here, and the Hand holds the temple: its best in front, and the rest
dead on the stairs below, where the things that live below caught them. At night the sky over the
isle is moths. Everything on the road that loves a lamp has come to the biggest one.

**New:** no family; the isle's new thing is the Hearth. **Back:** the moths at their most, and the
Hand at its best.

| Monster | Family | Role, level | Where | Look; what it does |
|---|---|---|---|---|
| Hearth Moth | moths | fodder, 28 | the isle, by night | *The sky over the isle is moving.* Flies; its dust puts to sleep (0.2); twelve to a group |
| Ashen Champion | cultist | soldier, 29 | the temple | *The Hand's best, in steel from below.* |
| Ashen Preacher | cultist | caster, 29 | the temple, behind the champions | *Still preaching, and the fire is the sermon.* Fire Bolt on a row; Ward on its group |

Its boss is the Core's.

### 9.2 The Underdeep (band 30–31)

*The hull's service layer, where geometry becomes too regular and materials too smooth. The final
dungeon descends through it from the temple.*

The Underdeep is the machine at work and nothing else: every machine family at its full size and on
its own ground, the keepers with no robes, knockers still inspecting walls that have not cracked in
four hundred years, and the vessel's defences, which caught the Hand on the stairs. And rats. They
are the only animals below the world, and nobody brought them: they came aboard the way rats do. A
company that met its first rat in a farm cellar meets its last one here.

**New:** crawlers, the long-legged things that walk the walls and ceilings. **Back:** every machine
family, at work; the rats.

| Monster | Family | Role, level | Where | Look; what it does |
|---|---|---|---|---|
| Deep Rat | rat | fodder, 30 | everywhere | *Rats. Even here.* Disease (0.2); twelve to a group |
| Keeper | keepers | soldier, 30 | the service ways | *A keeper with no bay to keep, and no robe.* |
| Deep Knocker | knockers | armoured, 30 | the corridors | *Still knocking, on a wall that has never cracked.* |
| Deep Crawler | crawlers, new | brute, 31 | walls and ceilings | *It comes down from above, on too many legs.* Reaches the back row |
| The Catcher | crawlers, new | boss, 31 | the stairs down to the core, among the Hand's dead | *It caught the Hand on these stairs.* |

### 9.3 The Core (band 32)

The last fight is people. Vask and his grey-handed men are halfway through putting out the light,
the Queen's signet on his finger and Wenna's palm held to the panel (STORY.md, Act Five). The signet
is the captain's line in a ring, so the core's machines answer it, and Vask calls them against the
company. The fight has a clock: through the roof, the light goes out a little every round until the
Grey Hands at the panel fall.

| Monster | Family | Role, level | Where | Look; what it does |
|---|---|---|---|---|
| Grey Hand | cultist | elite, 31 | the heart of the Hearth | *Hands the colour of ash, to the shoulder.* The Hand's inner circle, who walked Wenna from door to door |
| Core Sentry | heavy machines | soldier, 31 | called by the signet | *The core's own, and it takes orders from the ring.* |
| Lord Vask | the figure frame, a drawing of his own | boss, 32 | at the panel | *A tall man with a quiet voice, and the Queen's ring on his finger.* Calls sentries; never breaks |

The Custodian does not fight. When the Hand is broken it speaks, with one voice.

**Asks:** calls; the Core's clock.

---

## 10. The reach

The reach sits at the cap and asks the most (EXPANSION.md §5.2): a company at the cap that has
walked the road wins about half its first fights there, and what it finds is what makes the rest
winnable. Its super dungeons run many levels, each stranger than the one above. The two go opposite
ways, and so do their monsters: the Buried Tower down into what tried to leave, the Vault up into
what keeps the sky.

### 10.1 The Glass and the Buried Tower

The mast of a lander that tried to arrive, sunk in the glass it made, its decks running down into
the ground it melted (DESIGN.md §9). Its monsters are its own: the glass walkers at their worst,
the people who rode it down, still at their posts in the glass, and what grew in the melt.

| Monster | Family | Role, level | Where | Look; what it does |
|---|---|---|---|---|
| Glassbound | skeleton | soldier, cap | the Tower's decks | *In the glass to the chest, and still at its post.* |
| Glass Worm | long bodies | brute, cap | the melt under the decks | *A worm with a throat of glass.* |
| The Pilot | skeleton | boss, cap | the lowest deck | *Still in its seat, and still trying to leave.* |

The glass walkers come back at their full size on every deck.

### 10.2 Glacier Foot, the Ice Caves and the Vault

Where the glacier comes down from the rim and the sky comes down to meet the ice: the one place a
company can climb into the Vault, the inside of the sky, where the lights that make the sun and
the moon hang in the dark. The Ice Caves are the way up, and the Vault's monsters keep the lights.

| Monster | Family | Role, level | Where | Look; what it does |
|---|---|---|---|---|
| Ice Worm | long bodies | brute, cap | the Ice Caves | *It melted the cave it lives in.* |
| Rime Light | lights | controller, cap | where the ice is thinnest | *Cold light, drifting.* Its touch takes spell points |
| Lamplighter | keepers | soldier, cap | the Vault's gantries | *On its way to the sun, and you are on the gantry.* |
| The Old Lamplighter | keepers | boss, cap | beside the sun | *It lit the first morning.* |

---

## 11. The families, down the road

The ledger the novelty check reads (EXPANSION.md §5.4) and the creatures lane splits (EXPANSION.md
§8.1): each new family's module lands before the area that first needs it, the birds first, for
the pilot.

| Family | First met | Then |
|---|---|---|
| rat | the Shelf (built) | the bilge rat (Wrackholm), the deep rat (the Underdeep) |
| slime | the cellar (built) | none |
| boar | the Shelf (built) | none |
| wolf | the Shelf (built) | the black dog (the Downs), the sunder hound (Sunderwood), the moor hound (Cairnmoor) |
| spider | the Shelf (built) | the salt crab (Saltreach), the glass spider (Sunderwood), the fire beetle (the Kilns), the cinder beetle (Ashfall), the glass scorpion (the Wold) |
| bandit | the Shelf (built) | the wreckers and their lampman (the Downs), the bargemen (Saltreach), the Wrack smugglers (Wrackholm), the anvil guard (the Kilns) |
| cultist | the cellar (built) | the overseer (Wrackholm), the gleaner (Sunderwood), the mason (the Whitespine), the champion and the preacher (Hearth Isle), the Grey Hand (the Core) |
| skeleton | the cellar (built) | the barrow guard and captain (the Downs), the chanter and the Choirmaster (Saltreach), the bog body and the Cairn King (Cairnmoor), the ash husk (Ashfall), the glassbound and the Pilot (the Glass) |
| riftling | the cellar (built) | brine (Saltreach, Wrackholm), black glass (Sunderwood), slag (the Kilns): §2.1 |
| ogre | Thornmark (built) | the tor troll (Cairnmoor), the snow troll (the Whitespine) |
| wraith | Thornmark (built) | the cairn wight (Cairnmoor) |
| **birds** | the Downs | the great owl, the grey heron, the wrack gull, the raven, the spine eagle, the vulture |
| **old wood** | the Deepthorn | the strangler vine (Ashfall) |
| **long bodies** | Saltreach | the rock worm (the Kilns), the ice pike (Rimewater), the glass worm and the ice worm (the reach) |
| **toads** | Saltreach | none: the first to cut if the art budget bites |
| **devilfish** | Wrackholm | none |
| **moths** | Sunderwood | the hearth moth (Hearth Isle) |
| **bears** | Sunderwood | the ice bear (Rimewater) |
| **knockers** | the Kilns | the tallyman (Rimewater), the deep knocker (the Underdeep) |
| **salamanders** | the Kilns | the ember salamander (Ashfall), the basilisk (the Wold) |
| **lights** | Cairnmoor | the rime light (the Ice Caves) |
| **keepers** | Rimewater | the brothers, the bell-ringers and the Abbot, robed (the Whitespine); the keeper (the Underdeep); the lamplighters (the Vault) |
| **cats** | Rimewater | the Wold's lions |
| **giants** | the Whitespine | none |
| **drakes** | Ashfall | none |
| **heavy machines** | Ashfall | the core sentry (the Core) |
| **glass walkers** | the Wold | the Buried Tower |
| **crawlers** | the Underdeep | none |

In all, 105 new defs in 17 new families: with today's 33 in 11, 138 in 28, each def a drawing of
its own (EXPANSION.md §5.6). No area adds more than two families, and Hearth Isle and the Core add
none. Five families serve a single area: the toads and the devilfish, which can be cut, and the
giants, the drakes and the crawlers, which are their areas' centrepieces and cannot.

---

## 12. What this adds to the design

To take in or leave, as STORY.md's were:

- Five kinds, with `kind` on every monster in place of `mindless`'s two jobs, so that Holy Strike
  lands on the dead and nothing else.
- The machine's rules: each is doing a job; lightning bites; the Hearth's light passes through it,
  because the gods are the Custodian's voices and these are its hands.
- The Rift as a Stone's field torn loose and dressed in the Stone's material, with a shard for a
  heart; the Sunder staying open when its warden falls; no Rifts in the Whitespine, whose Stone is
  whole, or in Ashfall, whose Stone never made a field, so that on the far side the machine is near
  the surface instead.
- The dead gathering where a Stone is cut and lying down when it is restored, and the old wood
  waking and sleeping with the Grove Stone.
- No machine before the bottom of the Deep Mines and no Rift after Cairnmoor, held by a check.
- The keepers of the sleepers' bay putting people to sleep, and the monks as keepers under their
  robes, so that a company knows the walk.
- The Hand's hands greying with rank, as the Hand of Ash's are already drawn (`cultist.ts`), from
  the overseers' wrists to the Grey Hands' shoulders.
- The rats, the first thing killed and the last animal met.
- The moths, drawn to light from Lanternwood to the Hearth.
- The drowned chanters' count and the monastery's bells, both ending on the eleven.
- The giants' toll on the Stair.
- Vask's signet commanding the core's machines, and the Core's fight on a clock.

Where this and DESIGN.md disagree, the design wins.

### Open questions

1. **The Compact's crews.** The bargemen and the Wrack smugglers work for the Compact, whose line
   the company can take up (DESIGN.md §10.2). Either killing them sets the Compact against the
   company, or the crews who carry the Hand's cargo have left the Compact for the Hand's coin. The
   Compact's line should say which.
2. **What the giants are.** They are not among the sleepers, and every people came in the ship
   (DESIGN.md §7). Either they came awake, or they are something else. The monsters need only the
   surface; the design may want the rest.
3. **The reach's bands.** The atlas gives the Buried Tower 26–28 and the Ice Caves 20–22, where the
   reach is at the cap (DESIGN.md §9). The Ice Caves' way in lies in band-20 country, which is
   right; their monsters are the cap's.
4. **What a fight is worth.** From Saltreach on, an area spans two levels over as much as fifteen
   zone maps of land, where Thornmark spans five levels over fewer than six. At the slice's density
   of groups a clear would pay several bands, unless each fight pays far less of a level than
   Thornmark's do. The xp budget settles how much (EXPANSION.md §2.1, §5.2: however many fights an
   area holds, they share one budget, set so that three quarters of it reach the next floor), but it
   decides whether a fight on the road is worth having, and the country's floor of groups may want
   to sit looser than its floor of features (EXPANSION.md §5.3).
